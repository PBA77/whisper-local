...
class TranscriptionDiarizer:
    ...
    def load_models(self):
        ...
        if not self.hf_token:
            raise ValueError("HuggingFace token required. Set HF_TOKEN environment variable or pass --hf-token")
        ...
        try:
            ...
            print("Make sure you have accepted the terms for pyannote models on HuggingFace")
            raise
    ...
    def diarize_audio(self, audio_path: str, num_speakers: Optional[int] = None) -> Any:
        ...
        print(f"Diarization completed. Found {len(speakers)} speakers: {sorted(speakers)}")
        print(f"Diarization time: {diarization_time:.1f}s, Audio: {audio_duration:.1f}s, Speed: {diarization_ratio:.2f}x real-time")
    ...
    def merge_speaker_segments(self, diarization: Any, audio_duration: float) -> pd.DataFrame:
        ...
        for i in range(1, len(df)):
            ...
            if next_segment['speaker'] == current_segment['speaker']:
                print(f"  Merging {current_segment['speaker']}: {current_segment['end']:.1f}s → {next_segment['start']:.1f}s (gap: {gap:.1f}s)")
    ...
    def extract_audio_segment(self, audio_path: str, start_time: float, end_time: float, temp_dir: str) -> str:
        ...
        temp_file = os.path.join(temp_dir, f"segment_{start_time:.2f}_{end_time:.2f}.wav")
        torchaudio.save(temp_file, segment_waveform, sample_rate)
        
        return temp_file
    
    def transcribe_segments(self, audio_path: str, speaker_segments: pd.DataFrame) -> List[Dict[str, Any]]:
            ...
            for idx, segment in speaker_segments.iterrows():
                ...
                try:
                    ...
                    if text:  # Only add if transcription produced text
                        ...
                        print(f"  → Speed: {segment_ratio:.2f}x real-time ({transcription_time:.1f}s for {segment_duration:.1f}s audio)")
                    else:
        ...
        if total_transcription_time > 0:
            ...
            print(f"Overall transcription speed: {overall_ratio:.2f}x real-time ({total_transcription_time:.1f}s for {total_audio_duration:.1f}s audio)")
...
Examples:
  ...
  %(prog)s meeting.wav --format csv --hf-token YOUR_TOKEN
    ...
    if args.create_config:
        ...
        print("Configuration file created. Please edit it to add your HuggingFace token.")
        sys.exit(0)
    ...
    try:
        ...
        format_type = args.format if args.format != "json" else config.get_output_format()
